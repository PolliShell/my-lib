...
const addComment = async (req, res) => {
  ...
  if (!user) {
    return res
      .status(401)
      .json({ error: "Unauthorized. Please log in to add a comment." });
  }
...
const getCommentsByUserId = async (req, res) => {
  ...
  try {
    ...
    if (!comments.length) {
      return res
        .status(404)
        .json({ error: "Comments not found for this user" });
    }
...
const getReviewsByBookId = async (req, res) => {
  ...
  try {
    ...
    const reviews = await new Parse.Query(Reviews)
    ...
    const reviewsData = reviews.map((comment) => comment.toJSON());
